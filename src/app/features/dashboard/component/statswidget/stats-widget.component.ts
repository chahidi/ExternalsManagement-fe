...
export class StatsWidgetComponent implements OnInit {
    ...
    loadLanguagesChart(): void {
        ...
        const requests = this.languages.map((lang) => this.statsService.getCandidatesByLanguage(lang).pipe(catchError(() => of(0))));
    ...
    loadSkillsChart(): void {
        ...
        // Use forkJoin to handle multiple observables
        const requests = this.skills.map((skill) => this.statsService.getCandidatesBySkill(skill).pipe(catchError(() => of([]))));
    ...
    loadExperienceChart(): void {
        ...
        try {
            ...
            const yearsSuffix = this.translate.instant('dashboard.statistics.charts.experience.yearsSuffix');
            const labels = sorted.map(([label]) => label + (label !== '10+' ? yearsSuffix : yearsSuffix));
    ...
    private generateColors(count: number): string[] {
        const colors = ['#FF6F61', '#6B5B95', '#88B04B', '#F7CAC9', '#92A8D1', '#955251', '#B565A7', '#009B77', '#DD4124', '#45B8AC', '#EFC050'];
